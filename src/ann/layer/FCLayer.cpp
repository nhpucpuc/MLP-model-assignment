...
xt::xarray<double> FCLayer::backward(xt::xarray<double> DY) {
    ...
    auto batch_size = DY.shape()[0];
    auto N_in = this->m_aWeights.shape()[1];
    auto N_out = this->m_aWeights.shape()[0];

    xt::xarray<double> batch_grad_W = xt::zeros<double>({batch_size, N_out, N_in});
    
    for (size_t i = 0; i < batch_size; ++i) 
    {
        auto temp1 = xt::view(DY, i, xt::all());
        auto temp2 = xt::view(this->m_aCached_X, i, xt::all());

        xt::xarray<double> outer_product = xt::linalg::outer(temp1, temp2);
        xt::view(batch_grad_W, i, xt::all(), xt::all()) = outer_product;
    }
    m_aGrad_W = xt::sum(batch_grad_W, {0});
