...
V xMap<K,V>::remove(K key,void (*deleteKeyInMap)(K))
{
    ...
    for(auto x : list)
    {
        if(this->keyEQ(x->key, key))
        {
            ...
            if(deleteKeyInMap)
            {
                deleteKeyInMap(x->key);
            }
            list.removeItem(x, deleteEntry);
            count --;
            return retValue;
        }
    }
...
bool xMap<K,V>::remove(K key, V value, void (*deleteKeyInMap)(K), void (*deleteValueInMap)(V))
{
    ...
    for (auto x : list) 
    {
        if (this->keyEQ(x->key, key) && this->valueEQ(x->value, value))
        {
            if (deleteKeyInMap) 
            {
                deleteKeyInMap(x->key);
